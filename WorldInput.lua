...
local m_isSHIFTDown				:boolean= false;
...
function DefaultKeyDownHandler( uiKey:number )
	...
	if( pCity ~= nil ) then
		...
		if( uiKey == Keys.X ) then
			LuaEvents.CityPanel_TogglePurchaseTile();
		end
		if( uiKey == Keys.C ) then
			LuaEvents.CityPanel_ToggleManageCitizens();
		end
		if( uiKey == Keys.B ) then
			LuaEvents.CityPanel_TogglePurchaseWithGold();
		end
		if( uiKey == Keys.F ) then
			LuaEvents.CityPanel_TogglePurchaseWithFaith();
		end
	...
	if( uiKey == Keys.Q ) then
		LuaEvents.MapPinListPanel_ToggleMapPin();
		--LuaEvents.MinimapPanel_ToggleMapPinList();
	end
	-- END: place map pin

	-- START: toggle strategic map
	if( uiKey == Keys.V ) then
...
function OnMouseSelectionMove( pInputStruct:table )
	...
	if m_isMouseDragging then
		...
		if m_isMouseButtonLDown or m_isMouseButtonMDown then
